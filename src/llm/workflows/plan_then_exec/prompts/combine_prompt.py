def get_combine_prompt(
    ...
    - DATA-RICH SYNTHESIS: Synthesize the gathered information you DO have. You are FORBIDDEN from omitting specific numerical values or comparative evidence present in the log. If the task cannot be fully answered due to a critical step failure, provide the partial answer clearly and state exactly what data could not be retrieved.
